...
function needsResolution(q: any, nowIso: string): boolean {
...
export async function GET(request: Request) {
  ...
  const unresolvedCount = ((rows ?? []) as any[]).filter((q) => needsResolution(q, nowIso)).length;

  // Letzter Cron-Lauf (optional; Analytics ist "best effort").
  let lastCronAt: string | null = null;
  let lastCronMeta: any | null = null;
  try {
    ...
    if (!cronErr && Array.isArray(cronRows) && cronRows.length) {
      lastCronAt = typeof (cronRows[0] as any)?.created_at === "string" ? (cronRows[0] as any).created_at : null;
      lastCronMeta = (cronRows[0] as any)?.meta ?? null;
    }
  } catch {
    // ignore
  }
